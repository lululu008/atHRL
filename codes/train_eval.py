...
from code.agents.ddpg import ddpg_agent, hierarchical_ddpg, hierarchical_trajectory_ddpg, t_ddpg_agent
from code.agents.latent_sac import latent_sac_agent
from code.environments import filter_observation_wrapper
from code.networks import multi_inputs_actor_rnn_network
from code.networks import multi_inputs_critic_rnn_network
from code.networks import sequential_latent_network
from code.utils import gif_utils, h_driver, t_driver, ht_driver
from code.utils.common_utils import get_intention

os.environ["SDL_VIDEODRIVER"] = "dummy"

flags.DEFINE_string('root_dir', 'logs',
                    'Root directory for writing logs/summaries/checkpoints.')
flags.DEFINE_string('experiment_name', 'ht_ddpg_new',
...
def train_eval(
        ...
        model_network_ctor_type='hierarchical',  # model net
        ...
        intention_action_spec=tf_agents.specs.BoundedTensorSpec(shape=(2,), dtype=tf.float32, name="intention", minimum=0,
                                                                maximum=2),  # for H-DDPG
        intention_observation_spec=tf_agents.specs.TensorSpec(shape=(), dtype=tf.int32, name="intention"),
        trajectory_action_spec=tf_agents.specs.BoundedTensorSpec(shape=(3,), dtype=tf.float32, name="speed and point", minimum=[0.0, -1.0, 0.0],
                                                                maximum=[20.0, 1.0, 1.0]),
        # Params for collect
        ...
        train_checkpoint_interval=2500,
        policy_checkpoint_interval=2500,
        rb_checkpoint_interval=2500,
        log_interval=500,
    ...
    with tf.summary.record_if(
        ...
        if agent_name == 't_ddpg' or agent_name == 'ht_ddpg':
            intention_action_spec = tf_agents.specs.BoundedTensorSpec(shape=(3,), dtype=tf.float32, name="intention", minimum=0, maximum=2)
        ...
        elif agent_name == 'ddpg' or agent_name == 't_ddpg':
        ...
        if agent_name == 'h_ddpg' or agent_name == 'ht_ddpg':
            ...
            eval_intention_policy = intention_agent.policy
            eval_control_policy = control_agent.policy
