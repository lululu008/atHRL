...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params):
        ...
        # # Depth Camera sensor
        # self.camera_img_depth = np.zeros((self.obs_size, self.obs_size, 3), dtype=np.uint8)
        # self.camera_trans_depth = carla.Transform(carla.Location(x=0.8, z=1.7))
        # self.camera_bp_depth = self.world.get_blueprint_library().find('sensor.camera.depth')
        # # Modify the attributes of the blueprint to set image resolution and field of view.
        # self.camera_bp_depth.set_attribute('image_size_x', str(self.obs_size))
        # self.camera_bp_depth.set_attribute('image_size_y', str(self.obs_size))
        # self.camera_bp_depth.set_attribute('fov', '110')
        # # Set the time in seconds between sensor captures
        # self.camera_bp_depth.set_attribute('sensor_tick', '0.02')
        #
        # # Segmentation Camera sensor
        # self.camera_img_seg = np.zeros((self.obs_size, self.obs_size, 3), dtype=np.uint8)
        # self.camera_trans_seg = carla.Transform(carla.Location(x=0.8, z=1.7))
        # self.camera_bp_seg = self.world.get_blueprint_library().find('sensor.camera.semantic_segmentation')
        # # Modify the attributes of the blueprint to set image resolution and field of view.
        # self.camera_bp_seg.set_attribute('image_size_x', str(self.display_size))
        # self.camera_bp_seg.set_attribute('image_size_y', str(self.display_size))
        # self.camera_bp_seg.set_attribute('fov', '110')
        # # Set the time in seconds between sensor captures
        # self.camera_bp_seg.set_attribute('sensor_tick', '0.02')
    ...
    def reset(self):
        ...
        # self.camera_sensor_depth = self.world.spawn_actor(self.camera_bp_depth, self.camera_trans_depth, attach_to=self.ego)
        # self.camera_sensor_depth.listen(lambda data: get_camera_depth_img(data))
        #
        ...
        #     array = array[:, :, :3]
        #     array = array[:, :, ::-1]
        #     self.camera_img_depth = array
        #
        # # Add segmentation camera sensor
        # self.camera_sensor_seg = self.world.spawn_actor(self.camera_bp_seg, self.camera_trans_seg, attach_to=self.ego)
        # self.camera_sensor_seg.listen(lambda data: get_camera_seg_img(data))
        #
        ...
        # self.hero_polygons = []
        #
        # for ID, poly in vehicle_poly_dict.items():
        #     print(ID)
        #     if ID == self.birdeye_render.hero_id:
        #         self.hero_polygons.append(vehicle_poly_dict)
    ...
    def _get_obs(self):
        ...
        self.birdeye_render.render(self.display, birdeye_render_types, 'hero')
        birdeye_hero = pygame.surfarray.array3d(self.display)
        birdeye_hero = birdeye_hero[0:self.display_size, :, :]
        birdeye_hero = display_to_rgb(birdeye_hero, self.obs_size)
        ...
        if self.display_route:
            birdeye_render_types.append('waypoints')
        self.birdeye_render.render(self.display, birdeye_render_types, 'actor')
        ...
        birdeye_hero_surface = rgb_to_display_surface(birdeye_hero, self.display_size)
        ...
        #     pygame.image.save(lidar_surface, "./keypoints/new/lidar/screenshot"+str(self.obs_count//15+41772)+".jpeg")
        ...
        # camera_surface_depth = rgb_to_display_surface(camera_depth, self.display_size)
        #
        # if self.obs_count % 8 == 0 and self.obs_count // 8 < 30000:
        #     pygame.image.save(camera_surface_depth, "./keypoints/new/depth/" + str(self.obs_count // 8 + 12854).zfill(5) + ".png")

        ## Display segmentation camera image
        # camera_seg = resize(self.camera_img_seg, (self.obs_size, self.obs_size)) * 255
        # camera_surface_seg = rgb_to_display_surface(camera_seg, self.display_size)
        #
        #
        # if self.obs_count % 8 == 0:
        #     if self.obs_count // 8 > 30000:
        #         pygame.image.save(camera_surface_seg, "./quit/quit" + str(self.obs_count // 8 + 12854).zfill(5) + ".png")
        #     else:
        #         pygame.image.save(camera_surface_seg, "./keypoints/new/segmentation/"+str(self.obs_count//8 + 12854).zfill(5)+".png")
        # Display on pygame
        pygame.display.flip()
        ...
        obs = {
            ...
            'birdeye_hero': birdeye_hero.astype(np.uint8),
            'state': state,
        }
